"""
CBraMod backbone with PatchEmbedding.
Copied from the BASED training repo (cbramod/models/cbramod.py).
Imports criss_cross_transformer from the local severity package.
"""
import torch
...
class PatchEmbedding(nn.Module):
    def __init__(self, in_dim, out_dim, d_model, seq_len):
        ...
        self.positional_encoding = nn.Sequential(
            nn.Conv2d(in_channels=d_model, out_channels=d_model,
                      kernel_size=(19, 7), stride=(1, 1), padding=(9, 3), groups=d_model),
        )
    ...
    def forward(self, x, mask=None):
        ...
        spectral = torch.fft.rfft(mask_x.float(), dim=-1, norm='forward')
        spectral = torch.abs(spectral).to(mask_x.dtype).contiguous().view(bz, ch_num, patch_num, 101)
